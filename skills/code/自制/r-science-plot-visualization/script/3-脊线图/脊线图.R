...
df_plot <- read_csv(file.path(data_dir, "df_plot.csv"), show_col_types = FALSE)

# 预览数据
knitr::kable(head(rent))
...
p_legend <- df_for_legend %>% 
  ...
  theme(plot.title = element_text(family = font_family, face = "bold", size = 9,
                                  hjust = 0.075),
