...
- For any plan/task/spec page content edits with rich formatting, first read `notion://docs/enhanced-markdown-spec` via MCP.
- Inline math must use strict `$...$` delimiters with no spaces directly after `$` or before `$`.
- Keep whitespace around inline math in prose; avoid attaching delimiters directly to adjacent words/punctuation where parsing may fail.
- Author intended LaTeX with single backslashes (e.g., `\zeta`, `\mathrm`, `\frac{1}{12}`), not doubly escaped literals.
- If writing through JSON payloads, apply escaping only for transport; the conceptual document text must remain canonical Notion-flavored Markdown + LaTeX.
- Callouts must use exact syntax:
  - Start: `::: callout {icon="⚠️" color="red_bg"}`
  - End: `:::`
- Never assume successful write means correct rendering. Always `Notion:notion-fetch` after updates and verify formula/callout integrity; repair immediately if malformed.
