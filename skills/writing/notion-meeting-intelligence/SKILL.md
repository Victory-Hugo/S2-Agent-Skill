...
- Before creating/updating formatted meeting pages, read `notion://docs/enhanced-markdown-spec` via MCP to confirm current syntax.
- For inline equations, use strict `$...$` form with no spaces immediately inside delimiters.
- Ensure surrounding prose has whitespace around inline math delimiters so Notion parsing is stable.
- Write LaTeX as single-backslash commands in intended output (e.g., `\alpha`, `\sum`, `\frac`). Avoid accidental double-escaped authoring.
- When using API JSON strings, escape only at the JSON transport layer; do not change the intended final LaTeX.
- For callouts, use exact Pandoc fenced-div syntax with matching open/close fences:
  - `::: callout {icon="📌" color="yellow_bg"}`
  - `:::`
- After writing, run `Notion:notion-fetch` and validate equations and callouts render correctly. If you see escape artifacts or broken parsing, issue a corrective update in the same turn.
