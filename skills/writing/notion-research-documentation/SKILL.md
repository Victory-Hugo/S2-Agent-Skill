...
- When producing citation-heavy or math-heavy docs, first load `notion://docs/enhanced-markdown-spec` and follow it literally.
- Inline math must be `$...$` with no inner-edge spaces. Example: `$\zeta(-1)=-\frac{1}{12}$`.
- Keep whitespace around inline math delimiters in running text to satisfy Notion parser expectations.
- Preserve true LaTeX in authored content (`\sum_{n=1}^{\infty}`), and avoid over-escaping braces/commands.
- Distinguish output syntax from transport encoding: JSON escaping is allowed for payload safety, but intended rendered text must remain canonical LaTeX.
- Use callouts only with valid fenced-div syntax (`::: callout {...}` ... `:::`); do not improvise custom containers.
- Post-write validation is mandatory: fetch the page and confirm formulas, citations, and callouts are rendered (not serialized/escaped). Fix in-place if malformed.
